...
iso_files %>% 
  ...
  iso_filter_files(parse_number(Analysis) %in% c(534, 495)) %>% 
  # plot just mass 28
...
data_calibrated %>% 
  filter(compound %in% c("pristane", "phytane", "C19", "C21", "C23",  "C27", "C29", "C31", "C33"), true_d13C_pred_in_range == TRUE)%>%
  iso_plot_data(
    x = Depth, 
    y = true_d13C_pred, 
    y_error = true_d13C_pred_se
...
final_lowres <- data_calibrated %>% filter(Analysis != 493) %>% filter(Analysis != 495) %>%
  select(
      ...
      # peak info
       `Ampl 44`, true_d13C,  true_d13C_pred , true_d13C_pred_se, true_d13C_pred_in_range,  d13C_resid
