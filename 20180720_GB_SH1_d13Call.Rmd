...
final_data <- final_data %>%
    mutate(
         ...
         eps_p_lipid = 1000 * ((d13C_pool_avg + 1000)/ (d13C_p_avg + 1000) -1), # from Freeman and Hayes 1992, but using avg estimated d13C of DIC from lipid rather than CaCO3 measurement. This allows moving pool value - important when pool itself is experiencing characterizable d13C changes thru time. 
...
bulkorg <-  subset(alldata_w_time) %>%
  ...
  labs(y = "d13C") +
  scale_x_reverse(limits = c(94.5, 94.34)) +
  #scale_y_continuous(breaks = seq(-26, -23, len = 3)) +
...
pools <- alldata_w_time %>% 
  ...
  iso_plot_data(x = Date.Ma , y = d13C_pool_avg, color = compound)+
 geom_point(size = 8) +
