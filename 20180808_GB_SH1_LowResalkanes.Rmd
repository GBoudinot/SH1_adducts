---
title: "20180808_GB_A6 and low res aliphatics"
output:
  html_document:
    df_print: paged
editor_options:
...
```{r, echo = FALSE}
# load dataset
iso_files <- iso_read_continuous_flow("GB_SH1_lowres.cf.rda")
```
...
# remove 1 @ depth: 96.09 (OG028; 493), 94.56 (OG030; 495), 122 (OG005? )


iso_files %>% 
  # fetch a few of the files of interst
  iso_filter_files(parse_number(Analysis) %in% c(530, 532)) %>% 
  # plot just mass 28
...
```{r, echo = FALSE}
metadata_samples <- read_excel(file.path("metadata", "20180808_lowres_GB_metadata.xlsx"), sheet = "files")
metadata_peak_maps <- read_excel(file.path("metadata","20180808_lowres_GB_metadata.xlsx"), sheet = "maps")
metadata_samples
#add C31, 33
```
...
```{r "interactive1", fig.width = 8, fig.height = 8}
ggplotly()
```
...
```{r "interactive2", fig.width = 8, fig.height = 8}
ggplotly()
```
...
#Garrett thinks delta only looks best here
...
data_calibrated <- data_w_calibs %>% 
  # apply the delta_cross_ampl calibration (could apply multiple and compare)
