---
title: "20180720_GB_SH1_d13Call_maximum eps_f test"
output:
  html_document:
    df_print: paged
editor_options:
...
# Correlate lipid source by d13C
```{r}
cross_data <- final_data %>% 
  filter(true_d13C_pred_in_range == TRUE) %>% 
  filter(prep == "non-adduct") %>%
  filter(type == "sample") %>%
  filter(Identifier.1 != "OG004")%>%
  filter(Identifier.1 != "OG011")%>%
  group_by(Identifier.1) %>%
  filter(compound %in% c("pristane", "phytane", "C19", "C21")) %>%
  mutate(
    val.pristane = case_when(compound ==  "pristane" ~ true_d13C_pred),
    val.phytane = case_when(compound ==  "phytane" ~ true_d13C_pred),
    val.C19 = case_when(compound ==  "C19" ~ true_d13C_pred),
    val.C21 = case_when(compound ==  "C21" ~ true_d13C_pred)
      ) %>% select( Identifier.1, val.pristane, val.phytane, val.C19, val.C21) %>%  summarise_all(funs( na.omit(unique(.)) ))
  

crosslong_data <- final_data %>% 
  filter(true_d13C_pred_in_range == TRUE) %>% 
  filter(prep == "adduct") %>%
  filter(type == "sample") %>%
  filter(Identifier.1 != "OG038 AD") %>%
  filter(Identifier.1 != "OG039 AD") %>%
  filter(Identifier.1 != "OG163 AD") %>%
  group_by(Identifier.1) %>%
  filter(compound %in% c("C31", "C33", "C29")) %>%
  mutate(
    val.C29 = case_when(compound ==  "C29" ~ true_d13C_pred),
    val.C31 = case_when(compound ==  "C31" ~ true_d13C_pred),
    val.C33 = case_when(compound ==  "C33" ~ true_d13C_pred)
  ) %>% select( Identifier.1, val.C29, val.C31, val.C33) %>%  summarise_all(funs( na.omit(unique(.)) ))
...
grid.arrange(short, phyt,  ncol = 2)

table <- cross_data %>% 
  do({
    m <- lm(val.pristane ~ val.phytane, data = .)
    data_frame(
...
table3 <- crosslong_data %>% 
  do({
    m <- lm(val.C31 ~ val.C33, data = .)
    data_frame(
      offset = coefficients(m)[1],
      slope = coefficients(m)[2],
      rss = sum(summary(m)$residuals^2),
...
# lipid-leaf addition (second number in equation) by chain length from gymnosperm data in Diefendorf et al., 2011 [NOTE: average for gymnosperm is -2.5 per mil, that may be more appropriate]; minimum and maximum offset of leaf to atmosphere from C3 plant histogram in Ehleringer and Cerling, 2001 (in Taiz et al., 6th ed. Plant Physiology and Development)
  ...
  mutate(eps_net_min = case_when(.$compound %in% c("C17", "C19", "C21")  ~ (12+4) , #here using just the maximum "eps_f"
         ...
         eps_net_max = case_when(.$compound %in% c("C17", "C19", "C21")  ~ (25+8.4) , 
...
final_data <- final_data %>%
    mutate(
         ...
         eps_p_lipid = 1000 * ((d13C_pool_avg + 1000)/ (d13C_p_max + 1000) -1), # from Freeman and Hayes 1992, but using avg estimated d13C of DIC from lipid rather than CaCO3 measurement. This allows moving pool value - important when pool itself is experiencing characterizable d13C changes thru time. 
...
alldata_w_time <- alldata_w_time %>%
    mutate(
        Tk = 32 + 273.15, #T from O'brien et al 2017 d18O planktonic forams from ODP site 1050 (Blake Nose - closest in lat and long to WIS), norm to Kelvin
         ...
         eps_p_carb = 1000 * ((d13C_aq_CO2 + 1000)/ (d13C_p_max + 1000) -1) # from Freeman and Hayes 1992
         )
```
...
alldata_w_time %>% filter(compound == "C21") %>% select(eps_b, d13C_pool_avg, d13C_aq_CO2, eps_p_lipid, eps_p_carb) %>% kable()

```

## And extrapolate to pCO2 estimates with this
```{r}
# Assuming same values as above save inorganc carbon
alldata_w_time <- alldata_w_time %>%
    mutate(
