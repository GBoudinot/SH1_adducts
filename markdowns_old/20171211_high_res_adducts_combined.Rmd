...
```{r}
metadata_samples <- read_excel(file.path("metadata", "20171211_GB_metadata.xlsx"), sheet = "files")
metadata_peak_maps <- read_excel(file.path("metadata","20171211_GB_metadata.xlsx"), sheet = "maps")
...
data_w_peaks <- 
  ...
  iso_print_data_table (select = c(file_id, Analysis, `Identifier 1`, Depth), filter = !has_metadata, 
  ...
  iso_print_data_table(select = c(`Identifier 1`, file_id, compound, Start, Rt, End, Depth), filter = !is_identified | is_missing,
                       title = "Info: missing and unidentified peaks") %>% 
  # filter out the problematic peaks
  filter(is_identified, !is_missing, !is_ambiguous)

data_w_peaks_samples <- data_w_peaks %>% filter(type == "sample")
```
...
```{r}
data_w_peaks_samples %>% 
  ggplot() +
  aes(compound, y = `d 13C/12C`, color = file_id) +
  geom_point() +
...
#subtract 10 from d13C for now because of consistant offset

data_focused <- data_w_peaks %>% filter(Depth>115)

data_focused %>% 
  ggplot() +
  aes(x = Depth, y = `d 13C/12C`-10, color = file_id, size = `Ampl 44`) +
  geom_point() +
  scale_x_reverse() + 
