...
raw <- 
  bind_rows(
    read_excel(file.path("data", "20170822__data_export.xlsx")) 
  ) %>% 
...
# look at single run
### CHANGE ###
raw  %>% filter(File =="1561__170807_A5_PTV_1_5_A5dilutionsTest_CO2_newVial_14ng@ul.dxf")  %>% View()
```
...
data_matched <- map_peaks(raw, metadata_file = file.path("metadata", "smoky_hollow_peaks_August22.xlsx"), quiet = FALSE)
```
...
saveRDS(data_corrected, file = file.path("cache", "dataset_aug_corrected.rds"))
```
