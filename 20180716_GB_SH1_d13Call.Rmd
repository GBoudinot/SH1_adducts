---
title: "20180716_GB_SH1_d13Call_presentation"
output:
  html_document:
    df_print: paged
editor_options:
...
alldata_w_time <- alldata_w_time %>%
    mutate(
        Tk = 35 + 273.15, #T from O'brien et al 2017 d18O planktonic forams from ODP site 1050 (Blake Nose - closest in lat and long to WIS), norm to Kelvin
         ...
         d13C_aq_CO2 = d13C - 1 + eps_b, # from Julio's unpublished text , using CaCO3 d13C from Keller et al (above chunk)
         eps_p_carb = 1000 * ((d13C_aq_CO2 + 1000)/ (d13C_p + 1000) -1) # from Freeman and Hayes 1992
...
## Pool composition estimates from lipids compared with CaCO3
```{r}
pool_DIC_lipid <- alldata_w_time %>% 
  filter(compound %in% c("C19", "C21"))%>%
  ggplot()+
  facet_grid(~compound, scales = "free", space = "free") + 
  geom_point(aes(x = Date.Ma , y = d13C_pool_avg)) +
  #geom_errorbar(aes(x = ka , ymin = d13C_pool_min, ymax = d13C_pool_max)) +
