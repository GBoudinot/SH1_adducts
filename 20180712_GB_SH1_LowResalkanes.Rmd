---
title: "20180619_GB_A6 and low res aliphatics"
output:
  html_document:
    df_print: paged
editor_options:
...
iso_files %>% 
  ...
  iso_filter_files(parse_number(Analysis) %in% c(534, 535)) %>% 
  # plot just mass 28
...
```{r, echo = FALSE}
metadata_samples <- read_excel(file.path("metadata", "20180619_lowres_GB_metadatab.xlsx"), sheet = "files")
metadata_peak_maps <- read_excel(file.path("metadata","20180619_lowres_GB_metadatab.xlsx"), sheet = "maps")
metadata_samples
```
...
#Garrett thinks delta only looks best here, in fact
...
data_calibrated %>% 
  filter(compound %in% c("pristane", "phytane",  "C19", "C21", "C23",  "C27", "C29", "C31", "C33"))%>%
