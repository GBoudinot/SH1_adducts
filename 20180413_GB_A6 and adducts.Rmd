...
final_data <- data_calibrated %>%
  select(
      # sample info
      `file_id`, `compound`, `Identifier 1`, depth = `Depth`,  `measurement_info`, `ampl_sample_mean.mV`, `file_datetime`, 
      # peak info
...
alkane <- alldata_w_time %>% 
  filter(compound %in% c("C21", "C23",  "C33"), true_d13C_pred_in_range == TRUE, ka > -20 & ka < 80)%>%
  iso_plot_data(
    x = ka, 
    y = true_d13C_pred, 
    y_error = true_d13C_pred_se
...
bulkorg <-  subset(alldata_w_time, ka > -20 & ka < 80) %>%
  iso_plot_data(
    x = ka, 
    y = d13c_org
  ) + 
